...
10. ~~Route walks around obstacles instead of cutting straight through them~~ — done: every walk (wandering, interaction approach, conversation standoff) now goes through `navigation.findApproachPath` (direct line if clear -> cheap single-waypoint detour -> real grid-BFS pathfinding with corner-cutting disallowed -> direct-line fallback only if all else fails). See the README's "Pathfinding" section for the full breakdown. Found and fixed a real bug along the way: `margin` was a fixed 76px regardless of viewport width, which could fragment the walkable area into disconnected pockets on a narrow viewport -- now scales with `bounds.width`. Also **found but did not fix** a separate, real bug: `bounds` (the village's measured width/height, which everything above depends on) is captured once, synchronously, at page load in `app.js`, and never recomputed on resize -- if the browser resizes after that capture, navigation silently keeps using stale dimensions for the rest of the session. Worth fixing next time someone's in `app.js`.
