...
**What this doesn't cover:** pathfinding quality depends entirely on `bounds` (the village's measured width/height) being correct at the moment each resident's `CharacterNavigationController` is constructed in `app.js`. `margin` scales with `bounds.width` (was a fixed 76px regardless of viewport width, which could eat most of the usable space on a narrow viewport and genuinely fragment the walkable area into disconnected pockets that no pathfinder — coarse grid or otherwise — can route between). But `bounds` itself is captured once, synchronously, at page load, and isn't recomputed on window resize; if the browser resizes (or the initial layout measurement races ahead of final CSS settling) after that capture, every resident's navigation silently keeps using the stale dimensions for the rest of the session. Confirmed this while testing today — worth fixing as a follow-up (recompute `bounds` on resize, or at least on next interaction) but out of scope for this pass.
